...
```{r}
plotCounts(dds, "ENSMUSG00000024378",intgroup = c("Status"))
```
...
For the top gene in our analysis the command would be:-

```{r eval=FALSE}
select(org.Mm.eg.db, keys="ENSMUSG00000024378",
...
```{r}
results.annotated <- left_join(results.ordered, ann,by="ENSEMBL")
results.annotated
...
```{r}
write.csv(results.annotated,file="virgin_vs_lactation_DESeq_annotated",row.names=FALSE)
```
...
```{r}
plotCounts(dds, "ENSMUSG00000024378",intgroup = c("Status"),main="Stard4")
```
...
```{r}
attributeNames <- c('ensembl_gene_id', 'entrezgene', 'external_gene_name','start_position','end_position','chromosome_name')
# run the query
annot <- getBM(attributes=attributeNames, 
               filters = "ensembl_gene_id", 
               values = results.ordered$ENSEMBL, 
               mart = ensembl)
annot <- filter(annot, !duplicated(ensembl_gene_id)) %>% 
  rename("ENSEMBL" = ensembl_gene_id)

results.annotated.biomart <- left_join(results.ordered, annot)
...
```{r}
plot_data <- plotCounts(dds, "ENSMUSG00000024378",intgroup = c("CellType","Status"),returnData = TRUE)
plot_data
...
```{r}
ggplot(plot_data, aes(x = CellType,y=count,col=Status)) + geom_point() + geom_jitter(width = 0.1) + facet_wrap(~CellType)
```
