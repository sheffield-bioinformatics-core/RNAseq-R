...
>
> 1. Repeat the same plot as above but with the added argument `returnData=TRUE` 
> e.g. `plotCounts(dds, "ENSMUSG00000000381",intgroup = c("Status"))`
> 2. You should now be able to visualise these data in `ggplot2` to make a more attractive plot
> 3. Repeat the volcano plot from above, but use a different colour to indicate which genes are significant with an adjusted p-value less than 0.05

![](images/counts_ggplot.png)
