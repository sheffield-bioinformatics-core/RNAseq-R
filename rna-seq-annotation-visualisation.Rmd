...
We now have a list of genes ordered according to their evidence for being differentially-expressed.
...
plot(results$log2FoldChange, -log10(results$padj),
     ...
     xlab="Log Fold Change",
       ylab="Significance")
degenes <- which(results$padj < 0.05)
points(results$log2FoldChange[degenes],-log10(results$padj)[degenes],
       col="red",
       pch=16)
...
```{r}
plotCounts(dds, "94352",intgroup = c("CellType"))
```
...
```{r}
plotCounts(dds, "94352",intgroup = c("CellType"),main="Loxl2")
```
...
> ## Challenge {.challenge}
> 1. Repeat the same heatmap as above, but for the top 100 genes
