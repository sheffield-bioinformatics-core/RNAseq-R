...
As the `fastq` files are compressed, we can't use the `cat`, `head` or `tail` commands to view the contents (try it and see what happens!). 

```{bash eval=FALSE}
zcat SRR1552444.fastq.gz | head -n4
```
...
```{bash eval=FALSE}
scp dcuser@ec2-34-238-162-94.compute-1.amazonaws.com:~/*.html ~/Desktop
```
...
Desktop `~/Desktop/fastqc_html`. 

You should see a status output like this:

~~~
SRR1552444_fastqc.html                                                                                                            100%  261KB 261.0KB/s   00:00    
SRR1552445_fastqc.html                                                                                                            100%  258KB 258.2KB/s   00:00    
SRR1552446_fastqc.html                                                                                                            100%  260KB 260.4KB/s   00:00    
SRR1552447_fastqc.html                                                                                                            100%  258KB 258.1KB/s   00:00   
~~~


Now we can go to the Destkop on our own laptop open the HTML files in a web browser (e.g. Firefox). 
