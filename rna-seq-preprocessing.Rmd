...
>
> 1. Rerun the plotPCA function and change the intgroup parameter so that both CellType and Status are shown on the plot.
...
group <- factor(group)
glMDSPlot(dds, labels=labels, groups=group, folder="mds")
```
...
> ## Discussion {.challenge}
>
> Look at the MDS plot and PCA coloured by cell type.
...
Hopefully we have spotted a potential sample swap in the dataset. Such errors are not uncommon when handling 
...
> 2. Run plotPCA and verify that the sample groups now look correct
>


```{r}
save(dds,sampleinfo,file="Robjects/preprocessing.Rdata")
```
