...
In this section we will tackle the tasks of *aligning* our fastq reads to a reference genome and *quantifying* (counting). 

![workflow](images/workflow.png)

There are two different workflows that can be used;

- align the reads to the reference *genome* and then count how many reads map to a set of reference transcripts
- quantify the transcripts directly from the fastq file

The first workflow has been more prevalent in the literature to date, but is more computationally intensive. Quantification has become more fashionable in recent years with methods such as [`salmon`](https://salmon.readthedocs.io/en/latest/index.html) and [`kallisto`](https://pachterlab.github.io/kallisto/) becoming popular.

We will demonstrate both methods, but for further analysis in R we will use counts that have been generated with `salmon`. 

A much reduced dataset will be used for illustration although the commands used 
...
The command `wget` can be used to download a file from an FTP site to a local directory if you know the path to the file (URL). This path could be obtained by first locating the file in Ensbeml and right-clicking to copy the link address.

The following commands should download reference data from Ensembl into the `ref_data` folder that we have just created. For 
...
## Workflow 1: Align and then count
...
The program creates a genome index simply by using the *FASTA* file of the
...
**Note that the following index procedure make take a few minutes to run**
...
There are numerous tools performing short read alignment and the choice of the aligner
typically depends by the analysis goals and requirements. We will use HISAT2. The index files of
Mouse genome GRCm38 chromosome 1  are in the directory `index/`. All of them have a file name starting with the `GRCm38_chr1_hisat` prefix.
...
The steps in producing a bam file for analysis are given below. 


```{bash eval=FALSE}
samtools view -bS aligned_reads/SRR1552444.sam > aligned_reads/SRR1552444.bam
samtools sort aligned_reads/SRR1552444.bam aligned_reads/SRR1552444.sorted
...
## Workflow 2: Quantify the transcripts with salmon
...
Salmon is able to quantify transcript expression by using a quasi-mappings algorithm. Quasi-mappings are
...
`salmon` also requires the user to create an index from the reference data. This time it requires a `fasta` file of the transcripts (rather than whole-genome fasta). It does not however need the file to be uncompressed. Again, we have to specify a *prefix* name for all the files that `salmon` is going to create. We will be using the whole set of transcripts for the mouse GRCm38 genome build.
...
> What file contains the quantifications? What are the various columns?
...
for i in {44..55}
do
salmon quant -i index/GRCm38_salmon --libType A -r SRR15524${i}.fastq.gz -o quant/SRR15524${i}
gzip quant/SRR15524${i}/quant.sf
done
